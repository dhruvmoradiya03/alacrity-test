"use client";
import { updateOrg } from "@/services/organization.api";
...
const EditOrganization = ({
  ...
  onCancel: () => void;
  record: any;
  onEditOrganization: (updatedOrg: any) => void;
}) => {
  ...
  useEffect(() => {
    if (record) {
      setOrganizationName(record.organizationName);
      setGstNumber(record.gstNumber);
      setGstCertificate(record.gstCertificate);
      setEmail(record.email);
      setCountryCode(record.countryCode);
      setMobile(record.phoneNumber);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await updateOrg(idToken, record.id, updateOrgParams);

      if (res) {
        message.success("Organization updated successfully");
        onEditOrganization(res);
        onCancel();
      } else {
        message.error("Failed to update organization");
      }
    } catch (error) {
      message.error("Error while updating organization");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-8">
      ...
      >
        ...
        >
          <h2 className="text-xl font-semibold mt-3">Edit Organization</h2>
        </div>
